...
def main():
    ...
    print "030 - Test Download file from Guest"
    iscsivm = "iscstore01"
    ...
    print "Test fetch_file_from_guest function"
    src = "c:\\testfile.txt"
    dest = "tesetfile223.txt"
    # # def fetch_file_from_guest(self, vm, username, password, src, dest):
    # fetch_file_from_guest(service_instance, vm, "administrator", "Passw0rd!", src, dest)
    ...
    dest_path = "testfile.txt34"
    srce_path = "c:\\testfile.txt"
    creds = vim.vm.guest.NamePasswordAuthentication(
        username="administrator", password="Passw0rd!")

    print "Creating fileinfo InitiateFileTransferFromGuest"

    try:
        fileinfo = content.guestOperationsManager.fileManager. \
            InitiateFileTransferFromGuest(vm, creds, srce_path)
        #resp = requests.get(fileinfo.url, verify=False)
        # print "fileinfo = ", fileinfo
        print "fileinfo.url = ", fileinfo.url
        print "--", fileinfo.url, "--"
        ...
        print "Fixing url - replace esxi host ip with * - "
        print "--", url, "--"
